...
import { STATUS_CONFIG, PRIORITY_CONFIG, daysBetween, toDateString } from '../../util/helpers';
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <Field
                                ...
                                className={`${selectClass} ${modalMode === 'edit' ? inputDisabledClass : ''}`}
                            >
                            ...
                            <div className="relative">
                                <div
                                    className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full ${
                                        STATUS_CONFIG[values.status]?.dot || 'bg-slate-400'
                            ...
                            <div className="relative">
                                <span
                                    className={`absolute left-3.5 top-1/2 -translate-y-1/2 text-sm font-bold ${
                                        PRIORITY_CONFIG[values.priority]?.color?.replace('bg-', 'text-').replace('-50', '-600') || 'text-slate-400'
                                    }`}
                                >
                            ...
                            <Field
                                ...
                                onChange={(e) => {
                                    ...
                                    if (e.target.value && values.dueDate) {
                                        const diff = daysBetween(e.target.value, values.dueDate) + 1;
                            ...
                            <Field
                                ...
                                onChange={(e) => {
                                    ...
                                    if (values.startDate && e.target.value) {
                                        const diff = daysBetween(values.startDate, e.target.value) + 1;
                                        setFieldValue('duration', Math.max(diff, 1));
                                    }
                                }}
                            />
                            ...
                            <Field
                                ...
                                onChange={(e) => {
                                    ...
                                    if (values.startDate) {
                                        ...
                                        setFieldValue('dueDate', toDateString(newDue));
                                    }
                                }}
                            />
                        </div>
                        ...
                        <div className="relative h-2.5 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                            <div
                                className={`absolute inset-y-0 left-0 rounded-full transition-all duration-300 ${
                                    values.progress === 100 ? 'bg-green-500' : 'bg-slate-900'
                                }`}
                        ...
                        {dependencies.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-3">
                                {dependencies.map(depKey => {
                                    ...
                                    const badgeClass = "inline-flex items-center gap-2 px-2.5 py-1 bg-white border border-slate-200 text-slate-700 rounded-lg text-xs font-semibold";
                                    const btnClass = "text-slate-400 hover:text-red-500 transition-colors";
                                    return (
                                        <span key={depKey} className={badgeClass}>
                                            <HiOutlineLink className="w-3 h-3 text-slate-500" />
                                            {depTask?.taskKey || depKey}
                                            <button
                                                type="button"
                                                onClick={() => setDependencies(prev => prev.filter(k => k !== depKey))}
                                                className={btnClass}
                                            >
