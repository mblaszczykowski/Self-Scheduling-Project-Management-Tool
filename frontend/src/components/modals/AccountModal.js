...
const AccountModal = ({ user, onClose, onUpdateUser }) => {
    ...
    return (
        <Modal
            ...
            className={`max-w-lg mx-auto mt-10 bg-white rounded-xl shadow-2xl outline-none z-[70] max-h-[90vh] overflow-y-auto transition-all duration-300 ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
            overlayClassName={`fixed inset-0 bg-black/50 flex items-center justify-center z-[60] transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
        >
            {/* Header */}
