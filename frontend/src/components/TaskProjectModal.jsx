...
import { getImageUrl, getFileInfo, formatDateTime, formatDate, getStatusConfig, getPriorityConfig, getAvatarColor, getAvatarInitials } from '../util/helpers';
...
const PreviewModal = ({ preview, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-[70]" onClick={onClose}>
...
const TaskProjectModal = ({ modalType, modalMode, project, task, onClose }) => {
    ...
    return (
        <>
            ...
            <div ref={modalRef} className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-[1400px] max-h-[90vh] bg-white shadow-2xl rounded-xl z-50 flex flex-col transition-all duration-300 ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                {/* Header */}
                                    ...
                                    {modalType === 'task' ? (
                                        <>
                                            ...
                                            <div className="mb-6">
                                                ...
                                                <div className="rounded-lg border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-slate-900 focus-within:border-slate-900 transition-all hover:border-slate-300">
                                                    <ReactQuill theme="snow" value={values.description || ''} onChange={val => setFieldValue('description', val)} placeholder="Add details about this task..." modules={quillModules} className="[&_.ql-toolbar]:border-0 [&_.ql-toolbar]:border-b [&_.ql-toolbar]:border-slate-100 [&_.ql-toolbar]:bg-slate-50/50 [&_.ql-container]:border-0 [&_.ql-editor]:min-h-[200px] [&_.ql-editor]:text-sm" />
                                                </div>
                                            </div>
                                        ...
                                        <>
                                            ...
                                            <div className="mb-6">
                                                <InputLabel>Description</InputLabel>
                                                <div className="rounded-lg border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-slate-900 focus-within:border-slate-900 transition-all hover:border-slate-300">
                                                    <ReactQuill theme="snow" value={values.description || ''} onChange={val => setFieldValue('description', val)} placeholder="Describe the project..." modules={quillModules} className="[&_.ql-toolbar]:border-0 [&_.ql-toolbar]:border-b [&_.ql-toolbar]:border-slate-100 [&_.ql-toolbar]:bg-slate-50/50 [&_.ql-container]:border-0 [&_.ql-editor]:min-h-[200px] [&_.ql-editor]:text-sm" />
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                {currentUser && (
                                                    <div className="flex gap-2">
                                                        ...
                                                        <button type="button" onClick={() => handleAddMember(values.newUserEmail, values, setFieldValue)} disabled={emailLoading} className="px-4 py-2.5 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-all text-sm font-semibold disabled:opacity-50">
                                                            {emailLoading ? '...' : 'Add'}
                                                        </button>
                                                    </div>
                                                )}
