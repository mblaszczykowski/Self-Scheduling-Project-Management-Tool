 ...
 */
function verticalCrossesBars(x, sy, ey, allBars) {
 ...
 */
function findClearMidY(sy, ey, x1, x2, allBars) {
    const minY = Math.min(sy, ey);
    const maxY = Math.max(sy, ey);
...
function buildArrowPath(sx, sy, ex, ey, allBars) {
    if (Math.abs(ey - sy) < 1) {
...
const DependencyOverlay = ({
    ...
    const compute = useCallback(() => {
        ...
        const barEls = el.querySelectorAll('[data-task-key]');
